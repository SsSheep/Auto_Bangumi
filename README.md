...
![剧集总览](docs/images/episode-overview.png)
...
![检查计划](docs/images/check-schedule.png)
...
![卡片悬停操作](docs/images/card-hover.png)
...
设置页四个面板底部各有一个测试按钮，按表单当前值实测（密钥为掩码/空时后端回退已保存配置）：

- **网络设置 → 测试 TMDB 连接 / 测试 Bangumi API 连接**：验证地址与 API Key（自动识别 v3 Key / v4 Token），走应用代理
- **下载设置 → 测试下载器连接**：验证 qBittorrent / aria2 地址与认证，附版本信息
