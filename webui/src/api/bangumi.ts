...
export const apiBangumi = {
  ...
  /**
   * 自动获取指定番剧的放送星期（bgm.tv 日历优先，TMDB 兜底）
   */
  async refreshWeekday(bangumiId: number) {
    const { data } = await axios.get<ApiSuccess>(
      `api/v1/bangumi/refresh/weekday/${bangumiId}`
    );
    return data;
  },

  /**
   * 获取指定 bangumiId 的规则详情
   */
  async getOne(bangumiId: number) {
