...
describe('Download API contract (path + HTTP method)', () => {
  ...
  it('should pass download_now=false as query param when immediate download is off', async () => {
    (axios.post as any).mockResolvedValue({ data: mockApiSuccess });
    await apiDownload.subscribe(mockBangumiRule, mockRSSItem, false);
    expect(axios.post).toHaveBeenCalledWith(
      'api/v1/rss/subscribe',
      expect.anything(),
      { params: { download_now: false } }
    );
  });
});
