...
function groupNeedsReview(group: BangumiGroup) {
  return group.rules.some((r) => r.needs_review);
}

// ---------- 剧集总览入口（卡片悬停按钮） ----------
const episodesShow = ref(false);
const episodesRssId = ref<number | null>(null);
const episodesRssName = ref('');
let rssListCache: RSS[] | null = null;

async function onCardEpisodes(group: BangumiGroup) {
  const link = group.primary?.rss_link?.[0];
  if (!link) return;
  try {
    if (!rssListCache) rssListCache = await apiRSS.get();
    const rss = rssListCache.find((r) => r.url === link);
    if (!rss) {
      message.info(t('rss.episodes_no_rss'));
      return;
    }
    episodesRssId.value = rss.id;
    episodesRssName.value = rss.name || rss.url;
    episodesShow.value = true;
  } catch (e) {
    console.error('Failed to open episodes:', e);
  }
}
</script>
...
<template>
  <ab-pull-refresh :loading="refreshing" @refresh="onRefresh">
    ...
    <ab-rss-episodes-modal
      v-model:show="episodesShow"
      :rss-id="episodesRssId"
      :rss-name="episodesRssName"
    />
  </ab-pull-refresh>
</template>
