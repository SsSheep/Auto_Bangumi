...
import ConfigMediaLibrary from '@/components/setting/config-medialibrary.vue';
...
const sections: ConfigSection[] = [
  ...
  {
    id: 'media-library',
    titleKey: 'config.media_set.title',
    component: ConfigMediaLibrary,
    groups: ['media_library'],
    keywords: ['jellyfin', 'media', 'library', 'emby'],
  },
  {
    id: 'notification',
...
function onScroll() {
  ...
  if (!container) return;
  // 滚动触底时直接高亮最后一个分区：末尾分区较矮时其顶边
  // 永远到不了锚线，逐段比较会停在上一分区（如"软件更新"停在"安全设置"）
  const atBottom =
    container.scrollTop + container.clientHeight >= container.scrollHeight - 4;
  if (atBottom && visibleSections.value.length) {
    activeSection.value =
      visibleSections.value[visibleSections.value.length - 1].id;
    return;
  }
  // offsetTop 依赖 offsetParent（容器未定位时会错位），
  // 用视口矩形相对容器的偏移计算，坐标系始终一致
...
function jumpTo(id: string) {
  ...
  if (!reduceMotion) {
    ...
    const releaseWhenArrived = () => {
      ...
      if (arrived) {
        isJumping.value = false;
        container.removeEventListener('scroll', releaseWhenArrived);
        if (jumpTimer) clearTimeout(jumpTimer);
      }
    };
    container?.addEventListener('scroll', releaseWhenArrived, {
      passive: true,
    });
  }
  el.scrollIntoView({
