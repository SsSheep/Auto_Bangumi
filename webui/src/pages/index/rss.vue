...
// 剧集总览弹窗：查看可下载/已下载/已在库的集数并手动下载
const episodesShow = ref(false);
const episodesRssId = ref<number | null>(null);
const episodesRssName = ref('');

function onShowEpisodes(item: { id?: unknown; name?: unknown }) {
  episodesRssId.value = Number(item.id);
  episodesRssName.value = String(item.name ?? '');
      ...
      return (
        <div flex="~ justify-center gap-x-2">
          ...
          <AbButton
            ...
            onClick={() => onShowEpisodes(rss)}
          >
            <div class="i-carbon-list-boxes" />
            <span>{t('rss.episodes')}</span>
          </AbButton>
        </div>
      );
    },
          ...
          <div class="rss-card-content">
            ...
            <div class="rss-card-actions" @click.stop>
              ...
              <AbButton
                ...
                variant="primary"
                :title="$t('rss.episodes')"
                @click="onShowEpisodes(item)"
              >
                <div class="i-carbon-list-boxes"></div>
                {{ $t('rss.episodes') }}
              </AbButton>
            </div>
          </div>
