...
class Bangumi(SQLModel, table=True):
    ...
    check_interval: Optional[int] = Field(
        default=None, alias="check_interval", title="检查订阅间隔（秒），空为跟随全局"
    )
    last_check_time: Optional[str] = Field(
        default=None, alias="last_check_time", title="上次检查订阅时间（ISO 8601）"
    )
    check_weekdays: Optional[str] = Field(
        default=None, alias="check_weekdays", title="每周检查的星期（0-6 逗号分隔），设置后按星期+时间检查"
    )
    check_time: Optional[str] = Field(
        default=None, alias="check_time", title="每周检查的时间 HH:MM（24 小时制，精确到分钟）"
    )
...
class BangumiUpdate(SQLModel):
    ...
    check_weekdays: Optional[str] = Field(
        default=None,
        alias="check_weekdays",
        title="每周检查的星期（0-6 逗号分隔），设置后按星期+时间检查",
    )
    check_time: Optional[str] = Field(
        default=None,
        alias="check_time",
        title="每周检查的时间 HH:MM（24 小时制，精确到分钟）",
    )
