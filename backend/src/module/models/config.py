...
class MediaLibrary(BaseModel):
    """Media-library (Jellyfin) integration settings.

    凭据支持 ``$VAR`` 环境变量展开；未启用或未配置时，剧集总览接口
    会把 ``in_library`` 一律返回 False 并标记 ``jellyfin_available`` 为
    False，前端据此隐藏"已在库"状态。

    两个下载跳过开关（默认关闭，保持旧行为）：
    - ``skip_on_subscribe``：订阅时勾选"立即下载现有集数"先查库，
      已在 Jellyfin 的集不下载；
    - ``skip_on_refresh``：后台轮询追新集时先查库，已在库的不下载
      （防止手动整理进媒体库后被重复下载）。
    匹配失败（找不到剧集/接口错误）一律视为不在库，照常下载。
    """

    jellyfin_enable: bool = Field(
        default=False, description="Enable Jellyfin integration"
    )
    jellyfin_host_: str = Field(
        default="", alias="jellyfin_host", description="Jellyfin server URL"
    )
    jellyfin_api_key_: str = Field(
        default="", alias="jellyfin_api_key", description="Jellyfin API key"
    )
    skip_on_subscribe: bool = Field(
        default=False,
        description="Skip episodes already in library when subscribing",
    )
    skip_on_refresh: bool = Field(
        default=False,
        description="Skip episodes already in library during RSS refresh",
    )

    @property
    def host(self) -> str:
        return _expand(self.jellyfin_host_)

    @property
    def api_key(self) -> str:
        return _expand(self.jellyfin_api_key_)

    def configured(self) -> bool:
        return self.jellyfin_enable and bool(self.host) and bool(self.api_key)
...
class Config(BaseModel):
    ...
    proxy: Proxy = Proxy()
    media_library: MediaLibrary = MediaLibrary()
