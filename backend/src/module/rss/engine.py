...
def _parse_check_weekdays(value: str | None) -> set[int]:
    """解析 check_weekdays（"0,3,6" → {0, 3, 6}），非法输入返回空集。"""
    if not value:
        return set()
    days: set[int] = set()
...
def _parse_check_time(value: str | None) -> tuple[int, int] | None:
    """解析 check_time（"20:30" → (20, 30)），非法输入返回 None。"""
    if not value:
        return None
...
def _weekly_due(
    now: datetime,
    weekdays: set[int],
    check_hm: tuple[int, int],
    last: datetime | None,
) -> bool:
    """每周定时是否到期（now/last 均为 aware datetime，按本地时区比较）。"""
    now_local = now.astimezone()
    if now_local.weekday() not in weekdays:
        return False
    scheduled = now_local.replace(
        hour=check_hm[0], minute=check_hm[1], second=0, microsecond=0
    )
    if now_local < scheduled:
        return False
    if last is None:
        return True
    return last.astimezone() < scheduled


def _interval_due(now: datetime, interval: int, last: datetime | None) -> bool:
    """固定间隔是否到期。"""
    if last is None:
        return True
...
def _bangumi_check_due(bangumi: Bangumi, now: datetime) -> bool:
    """单个订阅是否到达可检查时间。

    两种模式：
    - 每周定时（check_weekdays + check_time 同时有效时优先）：本地时间
      到达指定的 星期+时刻 后检查一次，当天已检查过则跳过；
    - 固定间隔（check_interval，秒）：距离上次实际检查不足间隔的
      本轮跳过；为空或非正数时跟随全局节奏。

    注意最小粒度仍为全局 RSS 轮询周期（program.rss_time）。
    """
    weekdays = _parse_check_weekdays(bangumi.check_weekdays)
    check_hm = _parse_check_time(bangumi.check_time)
    last = None
    if bangumi.last_check_time:
        try:
            last = datetime.fromisoformat(bangumi.last_check_time)
        except ValueError:
            last = None
        else:
    ...
    if weekdays and check_hm is not None:
        return _weekly_due(now, weekdays, check_hm, last)
...
def _rss_linked_bangumi(item: RSSItem, bangumi_list: list[Bangumi]) -> list[Bangumi]:
    """按规则的 rss_link 反查该 RSS 源关联的订阅规则。

    聚合源服务多个番剧、无法归属单一检查计划，返回空列表（调用方
    据此放弃按计划跳过，保持每轮抓取的旧行为）。
    """
    if item.aggregate:
        return []
    return [
        ...
        if not b.deleted
        and item.url in {link.strip() for link in (b.rss_link or "").split(",")}
    ]


def _rss_fetch_due(item: RSSItem, bangumi_list: list[Bangumi], now: datetime) -> bool:
    """该 RSS 源本轮是否需要抓取报文。

    关联订阅全部未到检查时间（每周定时/固定间隔）时返回 False，
    整轮跳过网络抓取；关联不到任何订阅（或聚合源）时恒为 True。
    """
    linked = _rss_linked_bangumi(item, bangumi_list)
    if not linked:
        return True
    return any(_bangumi_check_due(b, now) for b in linked)
...
class RSSEngine:
    ...
    async def _refresh_rss(
        ...
        if not rss_id:
            # 检查计划（每周定时/固定间隔）未到的源整轮跳过网络抓取：
            # 旧实现每轮仍会完整拉取报文、只在分发阶段跳过
            due_items = [
                item
                for item in rss_items
                if _rss_fetch_due(item, bangumi_list, now_dt)
            ]
            skipped = len(rss_items) - len(due_items)
            if skipped:
                logger.debug(
                    "Skip fetching %s RSS feeds (check schedule not due)", skipped
                )
            rss_items = due_items
        ...
        # 本轮到达检查时间、实际处理过新种子的订阅（用于 last_check_time）
        checked_bangumi: dict[int, Bangumi] = {}
        # 抓取成功的源：其关联且到期的订阅先记为"已检查"——即使本轮
        # 没有新种子也写 last_check_time，否则每周定时的订阅在计划日的
        # 剩余时间里会被每轮反复抓取
        for rss_item, (new_torrents, error) in item_results:
            if error is not None:
                continue
            for b in _rss_linked_bangumi(rss_item, bangumi_list):
                if b.id is not None and _bangumi_check_due(b, now_dt):
                    checked_bangumi[b.id] = b
        ...
        # 追新前按 Jellyfin 过滤：本轮到期订阅的新种子里，已在媒体库的
        # 集不投递，直接记为已下载（防手动整理进库后被重复下载）。
        # 匹配失败视为不在库，照常下载。
        jellyfin_skip_ids: set[int] = set()
        if settings.media_library.skip_on_refresh:
            ...
            candidates = [
                ...
                for _, new_torrents, _, matches in item_matches
                for torrent, matched in zip(new_torrents, matches)
                if matched is not None and matched.id in checked_bangumi
            ]
            if candidates:
                _, skipped_jf = await EpisodeOverviewService(
                    self.db
                ).filter_in_library(candidates)
                jellyfin_skip_ids = {id(t) for t in skipped_jf}
                for torrent in skipped_jf:
                    torrent.downloaded = True
                await self.db.torrent.upsert_all(skipped_jf)
        ...
        for rss_item, new_torrents, error, matches in item_matches:
            ...
            for torrent, matched_data in zip(new_torrents, matches):
                if matched_data:
                    if matched_data.id not in checked_bangumi:
                        if _bangumi_check_due(matched_data, now_dt):
                            checked_bangumi[matched_data.id] = matched_data
                        else:
                            logger.debug(
                                "Skip %s: check interval of %s not elapsed",
                                torrent.name,
                    ...
                    if id(torrent) in skip_ids:
                        ...
                        continue
                    if id(torrent) in jellyfin_skip_ids:
                        logger.debug(
                            "Skip %s: episode already in Jellyfin library",
                            torrent.name,
                        )
                        continue
            ...
            # Jellyfin 跳过的已在过滤阶段 upsert 过，避免重复插入)
            to_persist = [
                t
                for t in new_torrents
                if id(t) not in failed_ids and id(t) not in jellyfin_skip_ids
            ]
        ...
        # 记录本轮实际检查过的订阅时间，供 check_interval 判断使用
        for bangumi in checked_bangumi.values():
            bangumi.last_check_time = now
            self.db.add(bangumi)
    ...
    async def download_bangumi(self, bangumi: Bangumi, skip_in_library: bool = False):
        async with RequestContent() as req:
            ...
            if torrents:
                if skip_in_library:
                    # 先查 Jellyfin，库里已有的集不下载（失败视为不在库）。
                    # 过滤依赖种子的 bangumi_id，必须先关联再过滤。
                    for torrent in torrents:
                        torrent.bangumi_id = bangumi.id
                    from module.manager.episode_overview import (
                        EpisodeOverviewService,
                    )

                    async with Database() as jf_db:
                        torrents, skipped = await EpisodeOverviewService(
                            jf_db
                        ).filter_in_library(torrents)
                    if skipped:
                        # 跳过的种子记为已下载：否则下轮轮询（尤其关着
                        # skip_on_refresh 时）会再发现并重复下载
                        for torrent in skipped:
                            torrent.downloaded = True
                        await self.db.torrent.upsert_all(skipped)
                    if not torrents and skipped:
                        return ResponseModel(
                            status=True,
                            status_code=200,
                            msg_en=(
                                f"All episodes of {bangumi.official_title} "
                                "already in Jellyfin; nothing downloaded."
                            ),
                            msg_zh=(
                                f"{bangumi.official_title} 的现有集数均已"
                                "在 Jellyfin 媒体库中，未重复下载。"
                            ),
                        )
                async with DownloadClient() as client:
