...
def _add_column_if_missing(table: str, column: str, ddl: str) -> GuardedStatement:
    """加列守卫：表不存在（如仅 auth 的旧库）时视为已应用、跳过该语句。"""
...
MIGRATIONS: tuple[Migration, ...] = (
    ...
    Migration(
        25,
        "add per-subscription check interval to bangumi",
        (),
        all_checks(
            table_exists("bangumi"),
            column_exists("bangumi", "check_interval"),
            column_exists("bangumi", "last_check_time"),
        ),
        (
            _add_column_if_missing(
                "bangumi",
                "check_interval",
                "ALTER TABLE bangumi ADD COLUMN check_interval INTEGER",
            ),
            _add_column_if_missing(
                "bangumi",
                "last_check_time",
                "ALTER TABLE bangumi ADD COLUMN last_check_time VARCHAR",
            ),
        ),
    ),
    Migration(
        26,
        "episode tag manual overrides",
        (),
        all_checks(
            table_exists("episode_status_override"),
            index_exists("episode_status_override", "idx_eps_override"),
        ),
        (
            (
                """CREATE TABLE IF NOT EXISTS episode_status_override (
            ...
            (
                "CREATE UNIQUE INDEX IF NOT EXISTS idx_eps_override "
                "ON episode_status_override (bangumi_id, season, episode)",
                index_exists("episode_status_override", "idx_eps_override"),
            ),
        ),
    ),
    Migration(
        27,
        "weekly check schedule for bangumi subscriptions",
        (),
        all_checks(
            table_exists("bangumi"),
            column_exists("bangumi", "check_weekdays"),
            column_exists("bangumi", "check_time"),
        ),
        (
            _add_column_if_missing(
                "bangumi",
                "check_weekdays",
                "ALTER TABLE bangumi ADD COLUMN check_weekdays VARCHAR",
            ),
            _add_column_if_missing(
                "bangumi",
                "check_time",
                "ALTER TABLE bangumi ADD COLUMN check_time VARCHAR",
            ),
        ),
    ),
    Migration(
        28,
        "episode overview cache per rss subscription",
        (),
        table_exists("episode_overview_cache"),
        (
            (
                """CREATE TABLE IF NOT EXISTS episode_overview_cache (
                    rss_id INTEGER NOT NULL PRIMARY KEY,
                    payload TEXT NOT NULL DEFAULT '',
                    built_at VARCHAR NOT NULL DEFAULT ''
                )""",
                table_exists("episode_overview_cache"),
            ),
        ),
