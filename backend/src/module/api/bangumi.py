...
@router.get(
    path="/refresh/weekday/{bangumi_id}",
    response_model=APIResponse,
    dependencies=[Depends(get_current_user)],
)
async def refresh_weekday(bangumi_id: int, db: Database = Depends(get_db)):
    """为单部番剧自动获取放送星期（bgm.tv 日历优先，TMDB 兜底）。"""
    manager = TorrentManager(db)
    resp = await manager.refresh_weekday(bangumi_id)
    return u_response(resp)
