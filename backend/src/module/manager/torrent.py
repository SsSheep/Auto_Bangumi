...
class TorrentManager:
    ...
    async def refresh_weekday(self, bangumi_id: int) -> ResponseModel:
        """为单部番剧自动获取放送星期（bgm.tv 日历优先，TMDB 首播日期兜底）。"""
        bangumi = await self.db.bangumi.search_id(bangumi_id)
        if not bangumi:
            return ResponseModel(
                status_code=406,
        ...
        try:
            ...
            logger.warning("bgm.tv calendar lookup failed for %s: %s",
                           bangumi.official_title, e)

        if weekday is None:
            weekday = await self._weekday_from_tmdb(bangumi)
            if weekday is not None:
                source = "TMDB"
        ...
        if weekday is None:
            return ResponseModel(
                status_code=404,
                status=False,
                msg_en=f"Failed to detect air weekday for {bangumi.official_title}.",
                msg_zh=f"未能获取 {bangumi.official_title} 的放送星期。",
            )

        bangumi.air_weekday = weekday
        # 自动获取到的值不锁定，后续日历刷新仍可修正
        bangumi.weekday_locked = False
        await self.db.bangumi.update_all([bangumi])
        logger.info(
            "Auto-detected weekday %s for %s (source: %s)",
            weekday, bangumi.official_title, source,
        )
        return ResponseModel(
            status_code=200,
            status=True,
            msg_en=f"Air weekday detected: {weekday} (source: {source}).",
            msg_zh=f"已自动获取放送星期：{weekday}（来源：{source}）。",
        )

    async def _weekday_from_tmdb(self, bangumi) -> int | None:
        """用 TMDB 该季第一集的放送日期推算星期几（周一=0）。"""
        language = settings.rss_parser.language
        try:
            info = await tmdb_parser(bangumi.official_title, language)
            if info is None:
                return None
            async with RequestContent() as req:
                air_dates = await get_season_episode_air_dates(
                    info.id, bangumi.season, language, req
                )
        except Exception as e:
            logger.warning("TMDB weekday lookup failed for %s: %s",
                           bangumi.official_title, e)
            return None
        first = next((d["air_date"] for d in air_dates if d.get("air_date")), None)
        if first is None:
            return None
        return first.weekday()  # Monday=0 ... Sunday=6，与 air_weekday 编码一致
    ...
    async def refresh_metadata(self):
        ...
        async def _air_dates(tmdb_id: int, season: int) -> list[dict]:
            key = (tmdb_id, season)
            if key not in air_dates_cache:
                async with RequestContent() as req:
                    air_dates_cache[key] = await get_season_episode_air_dates(
                        tmdb_id, season, language, req
                    )
            return air_dates_cache[key]
        ...
        for bangumi in bangumis:
            ...
            if tmdb_info:
                ...
                # Fill missing air weekday from TMDB so the calendar board
                # stops showing these series under "unknown".
                if bangumi.air_weekday is None and not bangumi.weekday_locked:
                    try:
                        dates = await _air_dates(tmdb_info.id, bangumi.season)
                    except Exception as e:
                        logger.warning(
                            "TMDB air date lookup failed for %s: %s",
                            bangumi.official_title,
                            e,
                        )
                        dates = []
                    first = next(
                        (d["air_date"] for d in dates if d.get("air_date")), None
                    )
                    if first is not None:
                        bangumi.air_weekday = first.weekday()
                        weekday_count += 1

        if archived_count > 0 or poster_count > 0 or weekday_count > 0:
            await self.db.bangumi.update_all(bangumis)

        logger.info(
            f"Metadata refresh: archived {archived_count}, "
            f"updated posters {poster_count}, filled weekday {weekday_count}"
        )
        return ResponseModel(
            status_code=200,
            status=True,
            msg_en=f"Metadata refreshed. Archived {archived_count} ended series, updated {poster_count} posters, filled {weekday_count} air weekdays.",
            msg_zh=f"已刷新元数据。归档了 {archived_count} 部已完结番剧，更新了 {poster_count} 个海报，自动填充了 {weekday_count} 个放送星期。",
        )
