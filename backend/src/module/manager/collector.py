...
class SeasonCollector:
    ...
    async def subscribe_season(
        data: Bangumi, parser: str = "mikan", download_now: bool = True
    ):
        async with Database() as db:
            engine = RSSEngine(db)
            data.added = True
            ...
            if not download_now:
                logger.info(
                    "Subscribed %s without downloading existing episodes.",
                    data.official_title,
                )
                return ResponseModel(
                    status=True,
                    status_code=200,
                    msg_en=f"Subscribed {data.official_title} successfully.",
                    msg_zh=f"订阅 {data.official_title} 成功，已跳过下载现有集数。",
                )
            return await engine.download_bangumi(
                data, skip_in_library=settings.media_library.skip_on_subscribe
            )
